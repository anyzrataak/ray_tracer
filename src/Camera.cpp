...
void Camera::initialize() {
    ...
    const double viewportWidth = viewportHeight * ((double)Width / Height);
    cameraCenter = Vector_3(0, 0, 0);

    // U goes right across the viewport, V goes downward.
    ...
    // Offset by half a pixel to center pixel00 within its cell.
...
void Camera::writeColor(std::vector<unsigned char>& px, int x, int y, Vector_3 c) {
    ...
    unsigned char pxl[Px];

    // Apply gamma 2 correction before writing to the 8-bit buffer.
...
Vector_3 Camera::rayColor(const Ray& r, int depth, const Hittable& world) {
    // No more bounces allowed: return black.
    if (depth <= 0) {
    ...
    if (world.hit(r, Interval(0.001, std::numeric_limits<double>::infinity()), rec)) {
        ...
        if (rec.getMat() && rec.getMat()->scatter(r, rec, attenuation, scattered)) {
            return attenuation * rayColor(scattered, depth - 1, world);
        }
        return Vector_3(0, 0, 0);
    }

    // No hit: blend white and light blue based on ray's vertical direction.
...
void Camera::render(const Hittable& world, std::vector<unsigned char>& pixels) {
    ...
    // Scale accumulated sample colors down to the [0, 1] range.
