...
CSGInterval getInterval(const Hittable& obj, const Ray& r) {
    ...
    HitRecord rec1;
    if (!obj.hit(r, fullRange, rec1)) return res;
    HitRecord rec2;
    Interval afterFirst(rec1.t + 1e-4, std::numeric_limits<double>::max());
    if (!obj.hit(r, afterFirst, rec2)) return res;
    res.tEnter = rec1.t;
    res.tExit = rec2.t;
    res.recEnter = rec1;
    res.recExit = rec2;
...
bool CSGUnion::hit(const Ray& r, Interval rayT, HitRecord& rec) const {
    ...
    if (hitA && !hitB) { rec = recA; return true; }
    if (!hitA && hitB) { rec = recB; return true; }
    rec = (recA.t < recB.t) ? recA : recB;
    return true;
}
...
bool CSGIntersection::hit(const Ray& r, Interval rayT, HitRecord& rec) const {
    ...
    if (!rayT.surrounds(tStart)) return false;
...
bool CSGDifference::hit(const Ray& r, Interval rayT, HitRecord& rec) const {
    HitRecord recA;
    if (!left->hit(r, rayT, recA)) return false;
    CSGInterval ib = getInterval(*right, r);
    if (!ib.valid || recA.t < ib.tEnter || recA.t > ib.tExit) {
        rec = recA;
        return true;
    }
    HitRecord recExit;
